...
const couldList = reactive({
    list: []
})
const search = ref('')
const musicUrl = ref('')
const imgUrl = ref('https://p2.music.126.net/6y-UleORITEDbvrOLV0Q8A==/5639395138885805.jpg')
const imgUrl2 = reactive({
    img: [
        'https://p2.music.126.net/6y-UleORITEDbvrOLV0Q8A==/5639395138885805.jpg',
    ]
})
// 查找
const queryBtn = async (x: any) => {
    const { data } = await axios.get(
        'https://yxcr-music-api.vercel.app/cloudsearch' + `?keywords=${x}`
    );
    if (data) {
        console.log(data)
        console.log(data.result.songs)
        couldList.list = data.result.songs;
    }
}
//播放
const playMusic = async (id: number) => {
    const { data } = await axios.get('https://yxcr-music-api.vercel.app/song/url', {
        params: {
            id: id
        }
    })
...
onMounted(() => {
    ...
    let x = ref('')
    // x = count;
    // x = router.currentRoute.value.query.keyworks as string
    console.log(router.currentRoute.value.query.keyworks)
    console.log(router.currentRoute.value.query)
    queryBtn(router.currentRoute.value.query.keyworks);
})
...
<template>
    ...
    <el-tabs v-model="activeName" class="demo-tabs" @tab-click="handleClick">
        <el-tab-pane label="歌曲列表" name="Song">
            <ul class="search_ul">
                <li class="adobe-product " v-for="(item,index) in couldList.list" @dblclick="playMusic(item['id'])">
                    <div class="products">
                        <!-- 歌名 -->
                        {{item['name']}}
                    </div>
                    ...
                    <div class="button-wrapper ">
                        ...
                        <div class="clicksvg" v-if="item['mv']!=0">
        ...
        <el-tab-pane label="歌单" name="ListSonglist">ListSonglist</el-tab-pane>
        <el-tab-pane label="MV" name="MV">MV</el-tab-pane>
        <el-tab-pane label="专辑" name="Album">Album</el-tab-pane>
        <el-tab-pane label="热门评论" name="PopularComments">PopularComments</el-tab-pane>
    </el-tabs>
</template>    
